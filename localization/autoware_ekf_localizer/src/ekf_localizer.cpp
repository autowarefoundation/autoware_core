  ...
  twist_queue_(params_.twist_smoothing_steps, params_.max_twist_queue_size)
{
  ...
  pub_diag_ = node_->create_publisher<diagnostic_msgs::msg::DiagnosticArray>("/diagnostics", 10);
  ...
  service_trigger_node_ = node_->create_service<std_srvs::srv::SetBool>(
    "trigger_node_srv",
    std::bind(
      &EKFLocalizerCore::service_trigger_node, this, std::placeholders::_1, std::placeholders::_2),
    service_trigger_qos);
...
void EKFLocalizerCore::timer_callback()
{
  ...
  stop_watch_.tic();
  DEBUG_INFO(
    node_->get_logger(), "------------------------- start prediction -------------------------");
  ekf_module_->predict_with_delay(ekf_dt_);
  DEBUG_INFO(
    node_->get_logger(), "[EKF] predictKinematicsModel calc time = %f [ms]", stop_watch_.toc());
  DEBUG_INFO(
    node_->get_logger(), "------------------------- end prediction -------------------------\n");
