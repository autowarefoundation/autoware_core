...
TEST(CalculateStopDistanceTest, Phase3Stop_StandardThreePhaseStop) {
    ...
    ASSERT_TRUE(result.has_value());
    EXPECT_NEAR(result.value(), 39.91666666, epsilon);
}

// ---------------------------------------------------------
// 3. Acceleration Edge Cases
// ---------------------------------------------------------

TEST(CalculateStopDistanceTest, HandlesAlreadyExceedingDecelerationLimit) {
    // If the vehicle is braking at -6.0 but the limit is -4.0, 
    // the function clamps a0 to -4.0 to maintain valid math.
    // v0 = 10.0, a0 = -6.0 (clamped to -4.0), a_limit = -4.0. No delay.
    // Phase 2 is skipped (already at limit).
    // Phase 3: x = -v0^2 / (2*a_limit) = -100 / (2 * -4) = 12.5m.
    auto result = calculate_stop_distance(10.0, -6.0, -4.0, -10.0, 0.0);
    
    ASSERT_TRUE(result.has_value());
    EXPECT_NEAR(result.value(), 12.5, epsilon);
}
