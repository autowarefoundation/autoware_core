  ...
  const double & arc_length, const double & shift_length, const ShiftParameters & shift_parameters)
{
  ...
  if (T_j < 0.0 || T_a < 0.0 || jerk < 0.0 || (T_j / T_total) < 0.1) {
    RCLCPP_WARN_THROTTLE(
      ...
      T_j, T_a, jerk, max_lateral_acc, a_lim_lat);
  ...
  // ------------------------------------------------------
  // Segment s1
  const double s1 = std::min(T_j * v_0_lon + 0.5 * a_lon * T_j * T_j, L);
  const double v1 = v_0_lon + a_lon * T_j;

  // Segment s2
  const double s2 = std::min(s1 + T_a * v1 + 0.5 * a_lon * T_a * T_a, L);
  const double v2 = v1 + a_lon * T_a;

  // Segment s3 = s4
  const double s3 = std::min(s2 + T_j * v2 + 0.5 * a_lon * T_j * T_j, L);
  const double v3 = v2 + a_lon * T_j;

  // Segment s5
  const double s5 = std::min(s3 + T_j * v3 + 0.5 * a_lon * T_j * T_j, L);
  const double v5 = v3 + a_lon * T_j;

  // Segment s6
  const double s6 = std::min(s5 + T_a * v5 + 0.5 * a_lon * std::pow(T_a, 2), L);
  const double v6 = v5 + a_lon * T_a;

  // Segment s7
  const double s7 = std::min(s6 + T_j * v6 + 0.5 * a_lon * std::pow(T_j, 2), L);
...
tl::expected<std::pair<std::vector<double>, std::vector<double>>, ShiftError> shift_impl(
  const std::vector<double> & bases, const ShiftInterval & shift_interval,
  const ShiftParameters & shift_parameters)
{
  if (shift_interval.start < 0.0 || shift_interval.end < 0.0) {
    return tl::unexpected{ShiftError{"cannot shift negative interval"}};
  }

  const double shift_arc_length = std::abs(shift_interval.end - shift_interval.start);
  const bool shift_forward = shift_interval.end > shift_interval.start;
  // Calculate base lengths
  const auto [base_lon, base_lat] = sanitize_same_base(calc_base_lengths(
    shift_arc_length,               //
  ...
  for (const auto new_base : base_lon) {
    merged_bases.insert(new_base);
  }
  std::vector<double> new_bases;
  std::vector<double> shift_values;
  new_bases.reserve(bases.size() + base_lon.size());
  shift_values.reserve(bases.size() + base_lon.size());
  ...
  for (const auto s : merged_bases) {
    ...
    if (shift_forward) {
      ...
      if (s > shift_interval.start) {
        ...
        // middle
        shift_values.push_back(cubic_spline->compute(shift_interval.start - s));
      } else {
        // after shifted
        shift_values.push_back(cubic_spline->compute(shift_arc_length));
      }
  ...
  return std::make_pair(new_bases, shift_values);
}
