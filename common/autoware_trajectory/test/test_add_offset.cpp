...
TEST(add_offset, temporal_trajectory_lateral_offset)
{
  std::vector<TrajectoryPoint> points;
  points.push_back(make_temporal_point(0.0, 0.0, 0.0, 5.0, 0.0));
  points.push_back(make_temporal_point(1.0, 0.0, 0.0, 5.0, 0.2));
  ...
  // Position should be shifted to the left
  EXPECT_NEAR(offset_traj.compute_from_time(0.0).pose.position.x, 0.0, 1e-6);
  EXPECT_NEAR(offset_traj.compute_from_time(0.0).pose.position.y, 1.0, 1e-6);
