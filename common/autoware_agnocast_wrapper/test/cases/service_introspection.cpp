...
TEST_F(ServiceIntrospectionTest, ClientAcceptsEveryState)
{
  ...
  const AUTOWARE_CLIENT_PTR(ListParameters) client =
    node->create_client<ListParameters>("~/introspect");
  ...
  const auto walk_every_state = [&] {
    for (const auto state :
         {RCL_SERVICE_INTROSPECTION_METADATA, RCL_SERVICE_INTROSPECTION_CONTENTS,
          RCL_SERVICE_INTROSPECTION_OFF}) {
      client->configure_introspection(node->get_clock(), rclcpp::QoS(1), state);
    }
  };

  // Assert
  EXPECT_NO_THROW(walk_every_state());
#endif
}
...
TEST_F(ServiceIntrospectionTest, ServiceAcceptsEveryState)
{
  ...
  const AUTOWARE_SERVICE_PTR(ListParameters) service = node->create_service<ListParameters>(
    "~/introspect", [](
  ...
  const auto walk_every_state = [&] {
    for (const auto state :
         {RCL_SERVICE_INTROSPECTION_METADATA, RCL_SERVICE_INTROSPECTION_CONTENTS,
          RCL_SERVICE_INTROSPECTION_OFF}) {
      service->configure_introspection(node->get_clock(), rclcpp::QoS(1), state);
    }
  };
...
TEST_F(ServiceIntrospectionTest, ClientRejectsNullClock)
{
  ...
  // Act
  const auto configure_with_null_clock = [&](rcl_service_introspection_state_t state) {
    client->configure_introspection(nullptr, rclcpp::QoS(1), state);
  };

  // Assert: rejected in every state, OFF included.
  EXPECT_THROW(
    configure_with_null_clock(RCL_SERVICE_INTROSPECTION_CONTENTS), std::invalid_argument);
  EXPECT_THROW(configure_with_null_clock(RCL_SERVICE_INTROSPECTION_OFF), std::invalid_argument);
#endif
}
...
TEST_F(ServiceIntrospectionTest, ServiceRejectsNullClock)
{
  ...
  // Act
  const auto configure_with_null_clock = [&](rcl_service_introspection_state_t state) {
    service->configure_introspection(nullptr, rclcpp::QoS(1), state);
  };
...
TEST_F(ServiceIntrospectionTest, ClientRejectsKeepAll)
{
#if RCLCPP_VERSION_GTE(21, 0, 0)
  // Arrange
  const auto node = std::make_shared<Node>("keep_all_client");
  const AUTOWARE_CLIENT_PTR(ListParameters) client =
    node->create_client<ListParameters>("~/introspect");

  // Act
  const auto configure_with_keep_all = [&](rcl_service_introspection_state_t state) {
    client->configure_introspection(node->get_clock(), rclcpp::QoS(rclcpp::KeepAll()), state);
  };

  // Assert
  EXPECT_THROW(configure_with_keep_all(RCL_SERVICE_INTROSPECTION_CONTENTS), std::invalid_argument);
  EXPECT_THROW(configure_with_keep_all(RCL_SERVICE_INTROSPECTION_OFF), std::invalid_argument);
#endif
}

TEST_F(ServiceIntrospectionTest, ServiceRejectsKeepAll)
{
#if RCLCPP_VERSION_GTE(21, 0, 0)
  // Arrange
  const auto node = std::make_shared<Node>("keep_all_service");
  const AUTOWARE_SERVICE_PTR(ListParameters) service = node->create_service<ListParameters>(
    "~/introspect", [](
                      AUTOWARE_SERVER_REQUEST_PTR(ListParameters) &&,
                      AUTOWARE_SERVER_RESPONSE_PTR(ListParameters) &&) {});
  ...
  // Act
  const auto configure_with_keep_all = [&](rcl_service_introspection_state_t state) {
    service->configure_introspection(node->get_clock(), rclcpp::QoS(rclcpp::KeepAll()), state);
  };
