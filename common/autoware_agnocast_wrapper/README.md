...
On rclcpp 21 (Iron) and newer, a client or service handle also carries `configure_introspection(clock, qos, state)`, which forwards to the `rclcpp` or the Agnocast counterpart. That lets utilities written against `rclcpp::Node` — `autoware_component_interface_utils` in particular — enable ROS 2 service introspection on a handle without knowing which backend is behind it. It is **not declared** on Humble (rclcpp 16), which has no service introspection at all, so gate any call on `RCLCPP_VERSION_GTE(21, 0, 0)` as the wrapper does. A null clock throws `std::invalid_argument` in every build and for every state, including `OFF`: the wrapper handle rejects it before the backend sees it, because Agnocast rejects a null clock and `rclcpp` dereferences it. That is one of the few places where the two backends have been made to agree on an exception; the wrapper does not normalize exceptions in general, as the contract note on `Timer` in `timer.hpp` explains. The check lives on the wrapper handle, so a caller holding a plain `rclcpp::Client` or `rclcpp::Service` — what a `NodeT`-generic utility deduces when `NodeT` is `rclcpp::Node` — still gets the `rclcpp` behaviour. What the two backends still do differently is the QoS, which is only consumed where the event publisher is created, on a transition out of `RCL_SERVICE_INTROSPECTION_OFF`: only there does the Agnocast path reject a QoS it cannot use (`KeepAll`, as `std::invalid_argument`) or fail to load the event typesupport (`std::runtime_error`, once per handle), while `rclcpp` accepts both. A call that only moves between `METADATA` and `CONTENTS`, or that turns introspection off, looks at neither.
