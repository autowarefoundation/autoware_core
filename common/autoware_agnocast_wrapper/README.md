...
launched one way with `ENABLE_AGNOCAST=1` and another way without it. The `<autoware_node>` launch
action takes that decision, so a launch file states the node once instead of spelling out both
forms. It is the alternative to the container swap above: it leaves the container on rclcpp and
moves the node out of it, rather than replacing the container with an Agnocast one.

`autoware_agnocast_wrapper_register_node()` registers the `autoware_node_plugins` resource the
action reads, mapping the executable it creates to its component class and to the transport the
build was made for.

The action is a launch frontend extension, which is found through a setuptools entry point. This
package is `ament_cmake`, so the entry point comes from the `setup.cfg` that
`ament_python_install_package(... SETUP_CFG setup.cfg)` hands to `setup.py egg_info`.
