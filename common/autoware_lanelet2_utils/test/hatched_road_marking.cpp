...
TEST(HatchedRoadMarking, AdjacentPolygons)
{
  ...
  const auto sample_map_dir =
    ...
    "sample_map";
  const auto map_path = sample_map_dir / "hatched_road_marking.osm";

  const auto lanelet_map_ptr = load_mgrs_coordinate_map(map_path.string());
  ASSERT_TRUE(static_cast<bool>(lanelet_map_ptr));

  // Collect several lanelets known to be adjacent to hatched road markings
  lanelet::ConstLanelets lanelets;
  lanelets.emplace_back(lanelet_map_ptr->laneletLayer.get(48));  // relation id 48 in OSM
  lanelets.emplace_back(lanelet_map_ptr->laneletLayer.get(59));  // relation id 59
  lanelets.emplace_back(lanelet_map_ptr->laneletLayer.get(70));  // relation id 70
  ...
  for (const auto & container : adjacent.left) {
    for (const auto & poly : container) {
      left_ids.insert(poly.id());
    }
  }
  for (const auto & container : adjacent.right) {
    for (const auto & poly : container) {
  ...
  EXPECT_EQ(left_ids.count(537), 1U);
  EXPECT_EQ(left_ids.count(560), 1U);
  EXPECT_EQ(left_ids.size(), 2U);

  EXPECT_EQ(right_ids.count(488), 1U);
