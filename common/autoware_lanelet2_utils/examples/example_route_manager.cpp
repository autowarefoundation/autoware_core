...
void route_manager_main()
{
  ...
  // create
  {
    ...
    // get current_lanelet
    if (!route_manager_opt.has_value()) { std::cerr << "Failed to create route manager" << std::endl; return; }
  ...
  // update lanelet
  {
    ...
    {
      if (!route_manager_opt.has_value()) { std::cerr << "Failed to create route manager for update" << std::endl; return; }
      route_manager_opt =
        std::move(route_manager_opt.value())
          .update_current_pose(P1, ego_nearest_dist_threshold, ego_nearest_yaw_threshold);

      if (!route_manager_opt.has_value()) { std::cerr << "Failed to update current pose" << std::endl; return; }
      const auto & route_manager = route_manager_opt.value();
    ...
    {
      if (!route_manager_opt.has_value()) { std::cerr << "Failed to get route manager for lane change" << std::endl; return; }
      route_manager_opt = std::move(route_manager_opt.value()).commit_lane_change_success(P5);
      if (!route_manager_opt.has_value()) { std::cerr << "Failed to commit lane change" << std::endl; return; }
      const auto & route_manager = route_manager_opt.value();
  ...
  const auto route_manager_opt =
    lanelet2_utils::RouteManager::create(map_msg_, route_msg_, initial_pose_);
  if (!route_manager_opt.has_value()) { std::cerr << "Failed to create route manager for lanelet sequence queries" << std::endl; return; }
  const auto & route_manager = route_manager_opt.value();
  ...
  {
    {
      ...
      if (!lane_opt.has_value()) { std::cerr << "Failed to get closest preferred route lanelet" << std::endl; return; }
      auto lane = *lane_opt;
...

    {
      ...
      if (!lane_opt.has_value()) { std::cerr << "Failed to get closest route lanelet within constraints" << std::endl; return; }
      auto lane = *lane_opt;
