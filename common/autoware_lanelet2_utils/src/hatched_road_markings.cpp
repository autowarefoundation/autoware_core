...
{
  AdjacentHatchedRoadMarkings result;
  ...
  for (const auto & llt : lanelets) {
    ...
    if (!left_polygons.empty()) {
      result.left.emplace_back(std::move(left_polygons));
    }
    ...
    if (!right_polygons.empty()) {
      result.right.emplace_back(std::move(right_polygons));
    }
  }
