...
{
DetectedToPredictedObjectsConverter::DetectedToPredictedObjectsConverter(const rclcpp::NodeOptions & options)
: rclcpp::Node("detected_to_predicted_objects_converter", options)
{
  detected_objects_sub_ = create_subscription<autoware_perception_msgs::msg::DetectedObjects>(
    ...
    std::bind(&DetectedToPredictedObjectsConverter::detected_objects_callback, this, std::placeholders::_1));
  ...
  const autoware_perception_msgs::msg::DetectedObjects::SharedPtr detected_objects_msg)
{
  ...
  for (const auto & detected_object : detected_objects_msg->objects) {
    ...
    predicted_kinematics.initial_pose_with_covariance = detected_object.kinematics.pose_with_covariance;
    
    if (detected_object.kinematics.has_twist) {
      predicted_kinematics.initial_twist_with_covariance = detected_object.kinematics.twist_with_covariance;
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(autoware::perception_objects_converter::DetectedToPredictedObjectsConverter) 