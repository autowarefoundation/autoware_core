...
bool hasIntersection(std::set<lanelet::Id> & a, const std::set<lanelet::Id> & b)
{
  for (const auto & id : a) {
    if (b.find(id) != b.end()) {
      return true;
  ...
  const geometry_msgs::msg::Pose & ego_pose, const State & state) const
{
  ...
  switch (state) {
    case State::APPROACH: {
      ...
      auto connected_lanelet_ids =
        getConnectedLaneletIds(linked_lanelet_id_, *planner_data_->route_handler_);
        ...
        autoware::experimental::trajectory::crossed_with_constraint(
          ...
          [&](const autoware_internal_planning_msgs::msg::PathPointWithLaneId & point) {
            return hasIntersection(
              ...
              std::set<lanelet::Id>{point.lane_ids.begin(), point.lane_ids.end()});
          });
