...
: vehicle_info_(autoware::vehicle_info_utils::VehicleInfoUtils(node).getVehicleInfo())
{
  ...
  no_ground_pointcloud = Pointcloud(PointcloudPreprocessParams(node));
}
...
//         trajectory_point.pose, front_length, rear_length, vehicle_width + mask_lat_margin_
//         * 2.0);
//     });
...
//   pcl::PointCloud<pcl::PointXYZ>::Ptr far_away_pointcloud_ptr(new
//   pcl::PointCloud<pcl::PointXYZ>); search_pointcloud_near_trajectory(
...
std::pair<pcl::PointCloud<pcl::PointXYZ>::Ptr, std::vector<pcl::PointIndices>>
PlannerData::Pointcloud::preprocess_pointcloud(
  const std::vector<TrajectoryPoint> & raw_trajectory,
  const autoware::vehicle_info_utils::VehicleInfo & vehicle_info,
  const TrajectoryPolygonCollisionCheck & collision_check, const double ego_nearest_dist_threshold,
  const double ego_nearest_yaw_threshold)
{
  auto ret_pointcloud_ptr = std::shared_ptr<pcl::PointCloud<pcl::PointXYZ>>(&pointcloud);
  const auto create_single_cluster_indices = [](size_t num_points) {
  ...
  auto ret_clusters = create_single_cluster_indices(ret_pointcloud_ptr->size());

  std::cerr << "PlannerData::Pointcloud::preprocess_pointcloud: "
            << "ret_pointcloud size: " << ret_pointcloud_ptr->size() << " pointcloud size: " << pointcloud.size() << std::endl;
  ...
  // if (!preprocess_params_.enable_preprocessing) {
  //   //ダミーのクラスタリングを作る
  //   //返す
  // }

  // const auto crop_param = preprocess_params_.crop_box_by_trajectory_polygon;
  // if (crop_param.enable_crop_box) {
  //   // make trajectory polygon
  ...
  // const auto downsample_param = preprocess_params_.downsample_by_voxel_grid;
  // if (downsample_param.enable_downsample) {
  //   const auto input pointcloud_ptr = ret_pointcloud_ptr;
  //   ret_pointcloud_ptr = std::make_shared<pcl::PointCloud<pcl::PointXYZ>>();
  //   // downsample pointcloud
  //   pcl::VoxelGrid<pcl::PointXYZ> voxel_grid_filter;
  //   voxel_grid_filter.setInputCloud(pointcloud.makeShared());
  //   voxel_grid_filter.setLeafSize(
  //     downsample_param.voxel_grid_x, downsample_param.voxel_grid_y,
  //     downsample_param.voxel_grid_z);
  //   voxel_grid_filter.filter(pointcloud);
  // }

  // cosnt auto cluster_param = preprocess_params_.cluster_by_euclidean;
  // if (cluster_param.enable_clustering) {
  //   // cluster pointcloud
  //   pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
