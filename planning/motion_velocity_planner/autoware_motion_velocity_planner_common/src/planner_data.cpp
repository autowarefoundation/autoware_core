...
pcl::PointCloud<pcl::PointXYZ>::Ptr crop_by_monothilic_trajectory_polygon_box(
  ...
  const autoware::vehicle_info_utils::VehicleInfo & vehicle_info)
{
  ...
  // TODO(yuki takagi): monothilic crop_box filter should be cared on the ego_coordinate system.
}  // namespace autoware::motion_velocity_planner
...
std::vector<pcl::PointIndices> make_cluster_indices(
  const pcl::PointCloud<pcl::PointXYZ>::Ptr & input_pointcloud_ptr,
  const PointcloudPreprocessParams::ClusteringFiltering & clustering_params)
{
  ...
  const pcl::PointCloud<pcl::PointXYZ>::Ptr & input_pointcloud_ptr)
{
  ...
  for (size_t i = 0; i < input_pointcloud_ptr->size(); ++i) {
    ret_clusters[i].indices.push_back(i);
  }
  ...
  const double ego_nearest_dist_threshold, const double ego_nearest_yaw_threshold)
{
  ...
  if (
    filter_by_trajectory_param.enable_mothilic_crop_box ||
    ...
    const auto traj_polygons =
      autoware::motion_velocity_planner::polygon_utils::create_one_step_polygons(
        ...
        filter_by_trajectory_param.lateral_mask_margin,
        traj_poly_param.enable_to_consider_current_pose, traj_poly_param.time_to_convergence,
        traj_poly_param.decimate_trajectory_step_length);

    if (filter_by_trajectory_param.enable_mothilic_crop_box) {
      pcl::PointCloud<pcl::PointXYZ>::Ptr input_pointcloud_ptr = ret_pointcloud_ptr;
      ret_pointcloud_ptr = crop_by_monothilic_trajectory_polygon_box(
        input_pointcloud_ptr, filter_by_trajectory_param, traj_polygons, decimated_trajectory,
        vehicle_info);
    }
    if (filter_by_trajectory_param.enable_multi_polygon_filtering) {
      pcl::PointCloud<pcl::PointXYZ>::Ptr input_pointcloud_ptr = ret_pointcloud_ptr;
  ...
  if (downsample_params.enable_downsample) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr input_pointcloud_ptr = ret_pointcloud_ptr;
    ret_pointcloud_ptr = downsample_by_voxel_grid(input_pointcloud_ptr, downsample_params);
  }

  const auto & clusering_param = preprocess_params_.clustering_filtering;
  if (clusering_param.enable_clustering) {
    ret_clusters = make_cluster_indices(ret_pointcloud_ptr, clusering_param);
  } else {
