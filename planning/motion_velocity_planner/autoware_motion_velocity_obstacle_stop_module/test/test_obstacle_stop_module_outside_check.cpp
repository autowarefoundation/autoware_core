...
public:
  ObstacleStopModuleWrapper() : ObstacleStopModule()
  {
    ...
    options.arguments(
      {"--ros-args", "--params-file",
       ...
       ament_index_cpp::get_package_share_directory("autoware_core_planning") +
         "/config/motion_velocity_planner/motion_velocity_planner.param.yaml"});
    ...
    node_->declare_parameter("limit.max_acc", 2.0);
    node_->declare_parameter("limit.min_acc", -5.0);
    node_->declare_parameter("limit.max_jerk", 2.0);
    node_->declare_parameter("limit.min_jerk", -2.0);
  ...
  geometry_msgs::msg::Pose get_specified_time_pose_wrapper(
    const std::shared_ptr<PlannerData::Object> object, const rclcpp::Time & time,
    const rclcpp::Time & predicted_objects_stamp) const
  {
    return object->get_specified_time_pose(time, predicted_objects_stamp);
  }
...
protected:
  ...
  double dist_to_bumper_ = vehicle_info_.max_longitudinal_offset_m;
...
TEST_F(OutsideCutInObstacleTest, GetSpecifiedTimePoseStaticObject)
{
  ...
  auto pose_0s = module_.get_specified_time_pose_wrapper(object, current_time_, current_time_);
  EXPECT_DOUBLE_EQ(pose_0s.position.x, pose.position.x);
  EXPECT_DOUBLE_EQ(pose_0s.position.y, pose.position.y);

  auto pose_1s = module_.get_specified_time_pose_wrapper(
