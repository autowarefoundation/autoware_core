...
protected:
  void SetUp() override
  {
    ...
    auto time_keeper = std::make_shared<autoware_utils_debug::TimeKeeper>(debug_processing_time_detail_);
...
TEST_F(TestSmootherBase, ComputeVelocityLimitFromSteerRate)
{
  ...
  double velocity_limit = smoother_base->computeVelocityLimitFromSteerRate(steer_rate_ratio, limits);
